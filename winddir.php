										...
										<div class="row">
											...
											<div class="col-sm-8 col-md-8 col-lg-8" id="time_interval">
												 ...
												 <label id="timeint_t">Time interval for graphs</label><br>
														<label id="1minlabel" class="btn btn-default btn-sm disabled">
...
$(function defaultchart() {
	...
	var options = {
		...
		title: {
			text: 'Wind Direction and Speed<br>' + nmonth + " " + year,
			x: -20 //center
		},
		subtitle: {
			text: 'Source: ' + station, 
		...
		series: [{
			...
			type: 'area',						// Make the new plot fill the area
            color: '#2d98c4',
			lineWidth: 1,
		 ...
		 tooltip: {
            ...
            useHTML: true,
			valueSuffix: ' km/h',
            headerFormat: '<kbd>Wind Direction: {point.key}\xB0</kbd> <hr><table>',
        ...
        if(lineNo !== 0) {
           var x = + new Date(items[2],items[1],items[0],items[3],items[4]),
            ...
            if(!isNaN(mainPlotline) && !isNaN(x)){
                options.series[0].data.push([mainPlotline,secondaryPlotline]);
